<script>
    ...
    `);
        ...
        function setDataAndShowEdit(data) {
            ...
            $('#manage-form').attr('action', `{{route('employee.update')}}/` + user.id);
        ...
        $('#manage-form').on('submit', function (e) {
            ...
            if (!dataId) {
                AjaxRequestPromise(`{{ route("employee.store") }}`, formData, 'POST', {useToastr: true})
                    .then(response => {
                        if (response?.data) {
                            const {
                                id,
        ...
        $('tbody').on('change', '.change-status', function () {
            ...
            AjaxRequestPromise(`{{ route('employee.change.status') }}/${rowId}?status=${status}`, null, 'GET', {useToastr: true})
                .then(response => {
        ...
        $(document).on('click', '.changePwdBtn', function () {
            ...
            $('#manage-form-2').attr('action', `{{route('employee.update.password')}}/` + id);
